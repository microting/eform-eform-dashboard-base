...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("Microting.eFormDashboardBase")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn'
        ]
        process_repository('Microting.eFormDashboardBase.csproj', packages, 'eform-eform-dashboard-base')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-eform-dashboard-base -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
